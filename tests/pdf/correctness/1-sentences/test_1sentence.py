...
def test_find_sentences_in_pdf() -> None:
    """Test finding sentences from search.txt in source.pdf."""
    ...
    # [start] Check that sentences were found
    phrase_stats = analysis_results['phrase_stats']
    
    assert analysis_results['total_matches'] > 0, (
        f"No matches found. Expected to find sentences from {search_terms}"
    )
    
    # Check if search terms appear in phrase_stats (as lemmas) or in forms
    for search_term in search_terms:
        search_term_lower = search_term.lower()
        phrase_found = False
        
        for phrase_lemma, stats in phrase_stats.items():
            # Check if phrase lemma matches
            if phrase_lemma.lower() == search_term_lower:
                phrase_found = True
                break
        ...
        assert phrase_found, f"Sentence '{search_term}' not found in PDF"
    # [end]

    # [start] Save results
    project_root = get_project_root()
    results_dir = project_root / 'results' / 'test' / 'pdf' / '1-sentences'
    ...
    print(f"  Phrase stats keys (lemmas): {list(phrase_stats.keys())}")
    print(f"  Output PDF: {output_pdf_path}")
    print(f"  Stats JSON: {stats_json_path}")


if __name__ == '__main__':
    test_find_sentences_in_pdf()
    print("Test passed!")
