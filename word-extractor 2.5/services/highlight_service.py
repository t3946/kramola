...
def _reconstruct_paragraph_with_highlighting(
    ...
    if source_paragraph.style and hasattr(source_paragraph.style, 'name'):
        try:
            ...
            if style_name in new_paragraph.part.document.styles:
                new_paragraph.style = new_paragraph.part.document.styles[style_name]
        except Exception: pass
    if hasattr(source_paragraph, 'paragraph_format'):
        _safe_copy_paragraph_format(source_paragraph.paragraph_format, new_paragraph.paragraph_format)
    if not tokens: return
    current_token_idx = 0
    source_runs = list(source_paragraph.runs)
    run_char_positions = []
    current_char_pos = 0
    ...
    for match in matches:
        match_start_idx = match['start_token_idx']; match_end_idx = match['end_token_idx']
        match_type_val = match['type']
        if match_start_idx > current_token_idx:
            for i in range(current_token_idx, match_start_idx):
                token = tokens[i]; run_style = find_source_run(token['start'])
                nr = new_paragraph.add_run(token['text'])
                if run_style: _apply_run_style(run_style, nr, copy_highlight=True)
        text_parts = []
        for i in range(match_start_idx, match_end_idx + 1):
            token = tokens[i]; text_parts.append(token['text'])
            run_style = find_source_run(token['start'])
            nr = new_paragraph.add_run(token['text'])
            if run_style: _apply_run_style(run_style, nr, copy_highlight=True)
            try: nr.font.highlight_color = HIGHLIGHT_COLOR_DOCX
            except Exception: pass
        found_text = "".join(text_parts).strip()
        if match_type_val == 'phrase':
            lemma_key = match['lemma_key']; stats = phrase_stats[" ".join(lemma_key)]
            stats['count'] += 1; stats['forms'][found_text] += 1
        elif match_type_val == 'word':
            lemma_key = match['lemma_key']
            if lemma_key:
                stats = word_stats[lemma_key]; stats['count'] += 1
            ...
            while token_idx <= num_tokens - phrase_len:
                ...
                if not current_range_indices.intersection(matched_token_indices):
                    ...
                    if valid_window and tuple(window_lemmas) == phrase_lemma_tuple:
                        ...
                        token_idx = end_actual_token_idx + 1; continue # Перескакиваем
...
def analyze_and_highlight_docx(
    ...
    try:
        ...
        for section_idx, section in enumerate(source_doc.sections):
           ...
           target_section.start_type = section.start_type; target_section.orientation = section.orientation
           target_section.page_width = section.page_width; target_section.page_height = section.page_height
           target_section.left_margin = section.left_margin; target_section.right_margin = section.right_margin
           target_section.top_margin = section.top_margin; target_section.bottom_margin = section.bottom_margin
           target_section.header_distance = section.header_distance; target_section.footer_distance = section.footer_distance
        ...
        for element in source_doc.element.body:
             if isinstance(element, docx.oxml.text.paragraph.CT_P): body_elements.append({'type': 'p', 'el': element})
             elif isinstance(element, docx.oxml.table.CT_Tbl): body_elements.append({'type': 'tbl', 'el': element})

        # Обработка элементов
        for item in body_elements:
            el_type, element = item['type'], item['el']
            if el_type == 'p':
                source_p = next((p for p in source_doc.paragraphs if p._element is element), None)
                ...
                try:
                    ...
                    if not tokens: # Пустой параграф
                        ...
                        _reconstruct_paragraph_with_highlighting(new_p, source_p, tokens, matches, word_stats, phrase_stats)
                ...
                source_t = next((t for t in source_doc.tables if t._element is element), None)
                ...
                try:
                    ...
                    try: new_t.autofit = source_t.autofit
                    except AttributeError: pass
                    try: new_t.alignment = source_t.alignment
                    except AttributeError: pass
                    ...
                    for i, row in enumerate(source_t.rows):
                        ...
                        for j, cell in enumerate(row.cells):
                            ...
                            target_c = new_row_cells[j]; source_c = cell
                            ...
                            try: target_c.vertical_alignment = source_c.vertical_alignment
                            except Exception: pass
                                    ...
                                    if not tokens: # Пустой параграф ячейки
                                        if cell_p.style and hasattr(cell_p.style,'name'):
                                            try:
                                                ...
                                                if style_name in result_doc.styles: new_cp.style = result_doc.styles[style_name]
                                            except Exception: pass
                                        if hasattr(cell_p, 'paragraph_format'):
                                            _safe_copy_paragraph_format(cell_p.paragraph_format, new_cp.paragraph_format)
                                    else: # Непустой параграф ячейки
                                        # Используем переданные данные
                                        matches = _find_matches_in_paragraph_tokens(
                                            tokens, search_lemmas_set, search_stems_set, search_phrase_lemmas_map
                                        )
                                        _reconstruct_paragraph_with_highlighting(new_cp, cell_p, tokens, matches, word_stats, phrase_stats)
                                        total_matches_count += len(matches)
        ...
        final_ps = {phrase_lemma_str: {'c': d['count'], 'f': dict(d['forms'])} for phrase_lemma_str, d in phrase_stats.items()}
...
def analyze_and_highlight_pdf(
    ...
    for page_num, logical_words_on_page_fitz in enumerate(all_pages_logical_words_fitz):
        ...
        for word_data in logical_words_on_page_fitz:
            ...
            page_word_candidates.append({'text': word_text_fitz, 'rects': word_rects_fitz, 'source': 'fitz', 'confidence': 100.0 })
        ...
        if use_ocr:
            try:
                ...
                print(f"Page {page_num + 1}: ocr_page returned. Data is None: {ocr_data is None}, Matrix is None: {ocr_matrix is None}")
                ...
                if ocr_data is not None and not ocr_data.empty and ocr_matrix is not None:
                    ...
                    print(f"Page {page_num + 1}: _merge_hyphenated_ocr_words returned {len(processed_ocr_words)} words.")





                    try: inverse_ocr_matrix = ~ocr_matrix; matrix_valid = True
                    except ValueError: matrix_valid = False
                    if matrix_valid:
                        for word_info in processed_ocr_words:   
                            ocr_text = word_info['text']; ocr_conf = word_info['conf']
                            original_indices = word_info['original_indices']
                            if ocr_conf < MIN_OCR_CONFIDENCE_HIGHLIGHT: continue
                            ocr_rect_tuples = []; valid_coords_for_word = True
                            for index in original_indices:
                                try:
                                    ...
                                    pdf_rect = img_rect * inverse_ocr_matrix; pdf_rect.normalize()
                                    ...
                                    else: valid_coords_for_word = False; break
                                except (KeyError, ValueError, TypeError):
                                    valid_coords_for_word = False; break
        ...
        for candidate in unique_page_word_candidates:
            ...
            if is_match:
                ...
                overlaps_existing = False; check_bbox = candidate_bbox + (-1,-1,1,1)
                ...
                for existing_hl_rect in highlighted_rects_on_page:
                    if check_bbox.intersects(existing_hl_rect):
                        intersection = check_bbox & existing_hl_rect; intersection_area = intersection.get_area()
                ...
                if not overlaps_existing:
                    current_word_rects_highlighted = []; quads_added_count_word = 0
                    for rect_tuple in candidate_rects_tuples:
                        try:
                            ...
                            if highlight:
                                highlight.set_colors(stroke=HIGHLIGHT_COLOR_PDF); highlight.update(opacity=0.4)
                                quads_added_count_word += 1; total_highlight_actions += 1
        ...
        if search_phrase_lemmas_map and fitz_tokens_for_phrases:
            ...
            for phrase_lemma_tuple in sorted_phrase_keys:
                ...
                for i in range(len(fitz_tokens_for_phrases) - target_len + 1):
                    ...
                    window_lemmas = []; valid_phrase_lemmas = True
                        ...
                        if not overlaps_existing_highlight:
                            ...
                            for rect_tuple in phrase_rects_tuples:
                                try:
                                    ...
                                    if highlight_annot:
                                        highlight_annot.set_colors(stroke=HIGHLIGHT_COLOR_PDF); highlight_annot.update(opacity=0.4)
                                        quads_added_count_phrase += 1; total_highlight_actions += 1
                            ...
                            if quads_added_count_phrase > 0:
                                ...
                                stats_phrase['count'] += 1; stats_phrase['forms'][phrase_text.strip()] += 1
                                highlighted_rects_on_page.extend(current_phrase_rects_highlighted)
    # Завершение (без изменений)
    total_matches_combined = sum(d['count'] for d in word_stats.values()) + sum(d['count'] for d in phrase_stats.values())
    if total_highlight_actions > 0:
        try: doc.save(output_path, garbage=4, deflate=True, clean=True)
        except Exception:
            if doc: doc.close(); return None
    else:
        if os.path.exists(output_path):
...
def _get_bounding_rect(rects_list_of_tuples):
    ...
    for r_tuple in rects_list_of_tuples:
        try:
            x0, y0, x1, y1 = r_tuple
            if x0 < x1 and y0 < y1:
                 min_x0 = min(min_x0, x0); min_y0 = min(min_y0, y0)
...
def _rects_overlap_significantly(rects1_list_tuples, rects2_list_tuples, threshold=0.5):
    ...
    area1 = bbox1.get_area(); area2 = bbox2.get_area(); intersection_area = intersection_rect.get_area()
...
def _deduplicate_candidates(candidates):
    ...
    unique_candidates = []; processed_indices = set()
    ...
    for i in range(len(candidates)):
        ...
        if is_target_word:
            print(f"DEBUG _deduplicate: Processing current_candidate (i={i}): '{current_candidate.get('text')}', Source: {current_candidate.get('source')}, Conf: {current_candidate.get('confidence')}")
        # --- КОНЕЦ ЛОГА ---


        overlapping_group = [current_candidate]; indices_in_group = {i}
        for j in range(i + 1, len(candidates)):
            if j in processed_indices: continue
            other_candidate = candidates[j]
            if _rects_overlap_significantly(current_candidate['rects'], other_candidate['rects']):
                # --- ЛОГ ПЕРЕСЕЧЕНИЯ ---
                if is_target_word or "ШУЛЬМАН" in other_candidate.get('text', '').upper(): # Логируем, если один из них - наша цель
                    print(f"DEBUG _deduplicate:   Overlap found between i={i} ('{current_candidate.get('text')}') and j={j} ('{other_candidate.get('text')}', Src: {other_candidate.get('source')}, Conf: {other_candidate.get('confidence')})")
                # --- КОНЕЦ ЛОГА ---


                overlapping_group.append(other_candidate); indices_in_group.add(j)
        ...
        if is_target_word: # Если текущий кандидат - наша цель
            if winner == current_candidate:
                ...
                print(f"DEBUG _deduplicate:   '{current_candidate.get('text')}' (i={i}) LOST to '{winner.get('text')}' (Src: {winner.get('source')}, Conf: {winner.get('confidence')}) in its group.")
            ...
            for member_idx, member in enumerate(overlapping_group):
                print(f"DEBUG _deduplicate:     Member {member_idx}: '{member.get('text')}', Src: {member.get('source')}, Conf: {member.get('confidence')}")
        # --- КОНЕЦ ЛОГА ---
...
def _merge_hyphenated_ocr_words(ocr_df):
    ...
    words_df = ocr_df[ (ocr_df['level'].astype(int) == 5) & (pd.to_numeric(ocr_df['conf'], errors='coerce') >= MIN_CONF_FOR_MERGE) & (ocr_df['text'].notna()) & (ocr_df['text'].astype(str).str.strip() != '') ].copy()
    ...
    output_words = []; merged_indices = set(); word_indices = list(words_df.index)
    for i in range(len(word_indices)):
        idx1 = word_indices[i]
        if idx1 in merged_indices: continue
        row1 = words_df.loc[idx1]; text1 = str(row1['text']).strip()
        # --- ДОБАВИТЬ ЛОГ ЗДЕСЬ ---
        ...
        ends_with_hyphen = text1.endswith(HYPHEN_CHARS); found_merge = False
        if ends_with_hyphen:
            for j in range(i + 1, len(word_indices)):
                idx2 = word_indices[j]
                if idx2 in merged_indices: continue
                row2 = words_df.loc[idx2]; text2 = str(row2['text']).strip()
                ...
                if is_below and vertical_dist_ok and is_left_aligned:
                    ...
                    output_words.append({'text': merged_text, 'conf': min_confidence, 'level': 5,'is_merged': True, 'original_indices': [idx1, idx2]})
                    merged_indices.add(idx1); merged_indices.add(idx2)
                    found_merge = True; break
        if not found_merge:
            if "ШУЛЬМАН" in text1.upper() or "ЕКАТЕРИНА" in text1.upper():
                print(f"DEBUG _merge: Adding original word to output: text1='{text1}'")
         # --- КОНЕЦ ЛОГА ---
            output_words.append({'text': text1, 'conf': row1['conf'], 'level': 5, 'is_merged': False, 'original_indices': [idx1]})
    return output_words
